...
```{r read_text}
hitz <-  read_tsv(here::here("data", "billboard_songs.txt"))
```
...
ggplot(hitz, aes(x= artist, y = `weeks on chart`)) +
  geom_point()
```
```{r most_hits_by_artist}
hitz %>% 
  group_by(artist) %>% 
  summarise(n = n()) %>% 
  arrange(desc(n)) %>% 
  slice(1:10) %>% 
  ggplot() +
   geom_point(mapping = aes(x = n, y = artist))
...
hitz %>% 
  ...
  slice(1:5) %>% 
  ggplot(mapping = aes(y = artist, x = `weeks on chart`, fill = `weeks on chart`)) +
